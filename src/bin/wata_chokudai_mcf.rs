...
use icfpc2023::{candidate::get_all_candidate, *};
...
fn compute_cand(input: &Input) -> Vec<P> {
    ...
    let cand2 = get_all_candidate(input);
    for can in cand2 {
        cand_set.insert(can);
    }
    for (_, p) in cand {
        cand_set.insert(p);
        if cand_set.len() >= input.n_musicians() * 2 {
            break;
        }
    }
    dbg!(cand_set.len());
...
fn main() {
    ...
    for iter in 0.. {
        ...
        if false && rng.gen_range(0, cand.len() * input.n_instruments()) == 0 {
            let mut pos = vec![];
            for i in 0..cand.len() {
                if state.insts[i] != !0 {
                    pos.push(i);
